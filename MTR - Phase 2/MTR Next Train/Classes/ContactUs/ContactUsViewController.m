...
- (void)viewDidLoad
{
   ...
   //   _background.image = [UIImage imageNamed:[NSString stringWithFormat:@"bg.png"]];
...
-(void)reloadData{
   ...
   for(int i = [[_general_scroll_view subviews] count] - 1; i >= 0; i--){
      NSObject *t = [[_general_scroll_view subviews] objectAtIndex:i];
      if([t isKindOfClass:[UIView class]]){
         [(UIView*)t removeFromSuperview];
         t = nil;
      }
   }
   ...
   dispatch_async(dbQueue, ^{
      ...
      DEBUGMSG(@"_general_contact_array: %@", _general_contact_array)
      DEBUGMSG(@"_line_contact_array: %@", _line_contact_array)
      DEBUGMSG(@"_station_contact_array: %@", _station_contact_array)
...
-(void)constructGeneralSection{
   ...
   for(x = 0; _general_contact_array != nil && x < [_general_contact_array count]; x++){
      ...
      if(data_type != 2){
         ...
         if([[CoreData sharedCoreData].lang isEqualToString:@"en"]){
            ...
            text_view = [[[UITextView alloc] initWithFrame:CGRectMake(22, y_offset, 100, 68)] autorelease];
            text_view.userInteractionEnabled = NO;
            text_view.backgroundColor = [UIColor clearColor];
            text_view.font = [UIFont systemFontOfSize:12.6];
            ...
            text_view = [[[UITextView alloc] initWithFrame:CGRectMake(22, y_offset-50, 100, 58)] autorelease];
            text_view.userInteractionEnabled = NO;
            text_view.backgroundColor = [UIColor clearColor];
            text_view.font = [UIFont systemFontOfSize:14];
            
            
            //text_view.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:13];
         ...
         //IS_IPHONE5
         if (IS_IPHONE5  == YES) {
            UITextView *text_view = [[[UITextView alloc] initWithFrame:CGRectMake(22, y_offset-40, 100, 55)] autorelease];
            text_view.userInteractionEnabled = NO;
            text_view.backgroundColor = [UIColor clearColor];
            text_view.font = [UIFont systemFontOfSize:15];
            //            text_view.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:13];
         ...
         if(data_type == 0){
            ...
            UITextView *content_text_view = [[[UITextView alloc] initWithFrame:CGRectMake(143, y_offset, 135, 150)] autorelease];
            ...
            //            content_text_view.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:13];
            ...
            //ios 7 layout Alex phone
            if([[[UIDevice currentDevice] systemVersion]doubleValue] >= 7.0){
               //               [content_text_view sizeToFit];
               content_text_view.frame = CGRectMake(158, y_offset, 120, height);            }
         }
            ...
            // ios 7.0 layout Alex
            if ([[[UIDevice currentDevice] systemVersion ] floatValue] >= 7.0) {
               text_view.frame = CGRectMake(22, y_offset-102, 120, height);
            }
            
            //ios6
            //            if ([[[UIDevice currentDevice] systemVersion ] floatValue] <= 7.0) {
            //               text_view.frame = CGRectMake(22, y_offset-95, 120, height+20);
            //               text_view.backgroundColor = [UIColor redColor];
            //            }
         ...
         if(data_type == 1 || data_type == 3){
            ...
            //            text_view.backgroundColor = [UIColor yellowColor];
            
            // ios 7.0 layout Alex
            if ([[[UIDevice currentDevice] systemVersion ] floatValue] >= 7.0) {
               button.frame = CGRectMake(152, y_offset-95, 120, 22);
               
            }
            //            if ([[[UIDevice currentDevice] systemVersion ] floatValue] <= 7.0) {
            //               button.frame = CGRectMake(152, y_offset-105, 120, 22);
            //
            //            }
         }
      }
      //datatype == 2
         ...
         //         text_view.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:15];
         text_view.text = [[[_general_contact_array objectAtIndex:x] objectForKey:[NSString stringWithFormat:@"label_%@", _lang]] stringByAppendingString:@": "];
         [_general_scroll_view addSubview:text_view];
   ...
   if(x % 2 == 1){
      highlight_image_view = [[[UIImageView alloc] initWithFrame:CGRectMake(0, y_offset, 300, 300)] autorelease];
      //      highlight_image_view.backgroundColor = [UIColor redColor];
      highlight_image_view.image = [UIImage imageNamed:@"bg_highlight.png"];
      [_general_scroll_view addSubview:highlight_image_view];
   }
   ...
   // ios 7.0 layout Alex
   if ([[[UIDevice currentDevice] systemVersion ] floatValue] >= 7.0) {
      text_view.frame = CGRectMake(22, y_offset-105 , 256, text_view.contentSize.height);
   }
   ...
   if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0)
      _general_scroll_view.contentSize = CGSizeMake(300, y_offset -170);
   else
      _general_scroll_view.contentSize = CGSizeMake(300, y_offset );
   _general_scroll_view.bounces = NO;
   //   _general_scroll_view.alpha = 0;
...
-(void)clickToLink:(UIButton*)button{
   ...
   if(url != nil && [url length] > 0){
      ...
      //      [temp release];
      //   UIAlertView *_alert = [[UIAlertView alloc] initWithTitle:@"" message:nil delegate:self cancelButtonTitle:LOCALSTR(@"Cancel") otherButtonTitles:LOCALSTR(@"OK"), nil];
      //   [_alert show];
      //   [_alert release];
      ////   [Logger logCatId:@"section" Items:@"bank_link"];
      //   UIAlertView *telAlert = [[UIAlertView alloc] initWithTitle:nil message:nil delegate:self cancelButtonTitle:NSLocalizedString(([NSString stringWithFormat:@"cancel_%@", [CoreData sharedCoreData].lang]), nil) otherButtonTitles:NSLocalizedString(([NSString stringWithFormat:@"ok_%@", [CoreData sharedCoreData].lang]), nil), nil];
      //   telAlert.tag = TAG_TELEPHONE;
      //   [telAlert show];
      //   [telAlert release];
   }
...
-(IBAction)clickStationContactLineButton:(UIButton*)button{
   ...
   if (    [[[UIDevice currentDevice] systemVersion] doubleValue] >= 7.0
      ...
      UIView* whiteview = [[[UIView alloc]initWithFrame:CGRectMake(8 ,0, 305, 250)]autorelease];
      whiteview.layer.cornerRadius = 6;
      whiteview.backgroundColor = [UIColor whiteColor];
      [_station_contact_line_picker_view insertSubview:whiteview atIndex:0];
      _station_contact_picker_bg_view.frame = CGRectMake(0, 0, 320, 460);
      _general_button.enabled = NO;
   }
   
   if(isFive ==NO)
      _station_contact_picker_bg_view.frame = CGRectMake(0, 2, 320, 460);
   else
      _station_contact_picker_bg_view.frame = CGRectMake(0, 2, 320, 558);
...
-(IBAction)clickStationContactStationButton:(UIButton*)button{
   ...
   if (    [[[UIDevice currentDevice] systemVersion] doubleValue] >= 7.0
      ...
      whiteview.layer.cornerRadius = 6;
      whiteview.backgroundColor = [UIColor whiteColor];
      [_station_contact_station_picker_view insertSubview:whiteview atIndex:0];
      _station_contact_picker_bg_view.frame = CGRectMake(0, 0, 320, 460);
      _general_button.enabled = NO;
   }
